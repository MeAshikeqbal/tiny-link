    ...
    <main className="min-h-screen bg-linear-to-br from-background to-muted/20">
      <div className="container mx-auto px-4 py-12">
        ...
        <div className="mb-12">
          ...
          <Link href={link.code} target="_blank" rel="noreferrer" className="inline-flex items-center mt-4 text-sm text-primary hover:text-primary/80 font-mono break-all">
            ...
            <div className="group relative overflow-hidden rounded-2xl bg-card border border-border p-8 transition-all hover:border-primary/50 hover:shadow-lg">
              ...
              <div className="relative z-10">
                ...
                <div className="text-lg font-semibold text-foreground">
                  {link.lstClickedAt ? format(new Date(link.lstClickedAt), "MMM dd, yyyy") : "Never"}
                </div>
                <p className="text-xs text-muted-foreground mt-3">
                  {link.lstClickedAt ? format(new Date(link.lstClickedAt), "h:mm a") : "No activity yet"}
                </p>
              </div>
            </div>
          </div>

          {/* Target URL Card - Large */}
          <div className="lg:col-span-2 group relative overflow-hidden rounded-2xl bg-card border border-border p-8 transition-all hover:border-primary/50 hover:shadow-lg">
            ...
            <div className="relative z-10">
              ...
              <div className="flex gap-3">
                <CopyButtonClient text={link.targetUrl} />
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="group relative overflow-hidden rounded-2xl bg-card border border-border p-8 transition-all hover:border-primary/50 hover:shadow-lg">
          ...
          <div className="relative z-10">
            ...
            <div className="grid grid-cols-2 gap-8 md:grid-cols-4 md:gap-12">
              <div>
                <p className="text-2xl font-bold text-foreground">{format(new Date(link.createdAt), "dd")}</p>
                <p className="text-xs text-muted-foreground mt-2">{format(new Date(link.createdAt), "MMMM")}</p>
              </div>
              <div>
                <p className="text-2xl font-bold text-foreground">{format(new Date(link.createdAt), "yyyy")}</p>
                <p className="text-xs text-muted-foreground mt-2">Year</p>
              </div>
              <div>
                <p className="text-2xl font-bold text-foreground">{format(new Date(link.createdAt), "EEEE")}</p>
                <p className="text-xs text-muted-foreground mt-2">Day</p>
              </div>
              <div>
                <p className="text-2xl font-bold text-foreground">{format(new Date(link.createdAt), "h:mm a")}</p>
